...
sudo --preserve-env --login -u \$HOST_WHOAMI PYCVODES_LAPACK=lapack pip install --user -e .; \
sudo --preserve-env --login -u \$HOST_WHOAMI jupyter notebook --no-browser --port $PORT --ip=* --notebook-dir examples"

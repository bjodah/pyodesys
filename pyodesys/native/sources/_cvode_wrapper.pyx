...
cdef pair[pair[vector[double], vector[double]], unordered_map[string, int]] _integrate_adaptive(
    ...
    string iter_type, int linear_solver) nogil except+:
    cdef:
        OdeSys * odesys
    ...
    try:
        result = simple_adaptive[OdeSys](
            odesys, atol, rtol, lmm_from_name(lmm), y0, x0, xend, root_indices, mxsteps,
            dx0, dx_min, dx_max, with_jacobian, iter_type_from_name(iter_type), linear_solver)
...
cdef unordered_map[string, int] _integrate_predefined(
    ...
    bool with_jacobian, string iter_type, int linear_solver) nogil except+:
    cdef:
        OdeSys * odesys
        vector[int] root_indices
    ...
    odesys = new OdeSys(params)
    try:
        simple_predefined[OdeSys](odesys, atol, rtol, lmm_from_name(lmm), y0, nout, xout, yout,
                                  root_indices, root_out, mxsteps, dx0, dx_min, dx_max,
                                  with_jacobian, iter_type_from_name(iter_type), linear_solver)
...
def integrate_predefined(cnp.ndarray[cnp.float64_t, ndim=1] y0,
    ...
    nfo = _integrate_predefined(&y0[0], xout.shape[xout.ndim - 1], &xout[0], &yout[0, 0],
                                ...
                                with_jacobian, iter_type.lower().encode('UTF-8'), linear_solver)
...
def integrate_adaptive(cnp.ndarray[cnp.float64_t, ndim=1] y0,
    ...
    (xout, yout), nfo = _integrate_adaptive(
        ...
        atol, rtol, method.lower().encode('UTF-8'), dx0, dx_min, dx_max, mxsteps, with_jacobian,
        iter_type.lower().encode('UTF-8'), linear_solver)

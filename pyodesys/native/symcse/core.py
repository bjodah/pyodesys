...
class NullTransformer:
    ...
    def statements_with_declarations(
            self,
            pred=lambda s: "[" not in s.name,
    ...
    def _as_decl(ctx_seen, statements, is_const, *, pred, type_, lift=False):
        ...
        for st in statements:
            if isinstance(st, Assignment) and st.lhs not in ctx_seen and pred(st.lhs):
                ctx_seen.add(st.lhs)
                t, new_rhs = type_(st.lhs, st.rhs)
                var = Variable(
                    st.lhs,
                    type=t,
                    attrs=[value_const] if is_const[st.lhs] == 1 else [],
                )
                if lift:
                    lifted.append(var.as_Declaration())
                else:
                    st = var.as_Declaration(value=new_rhs)
            elif hasattr(st, "body"):
                body_stmnts, body_lifted = NullTransformer._as_decl(
                    ctx_seen, st.body.args, is_const, pred=pred, type_=type_, lift=True
                )
                if lift:
    ...
    def _declare(self, stmts, *, pred=lambda x: True, type_=(lambda lhs, rhs: (float64, rhs))):
        seen = set()
        is_const = defaultdict(int)
        self._is_const(is_const, stmts)
        new_stmnts, lifted = self._as_decl(
            seen, stmts, is_const, pred=pred, lift=False, type_=type_
        )
        assert len(lifted) == 0
        return new_stmnts

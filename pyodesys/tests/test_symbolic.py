...
def test_SymbolicSys_as_autonomous__to_arrays(auto):
    ...
    odes = SymbolicSys.from_callback(rhs, 2, 1, to_arrays_callbacks=(
        lambda t: [_t.rescale(pq.s).magnitude for _t in t],
        lambda y: [_y.rescale(pq.molar).magnitude for _y in y],
        None
    ))
...
def test_SymbolicSys_from_other_new_params_by_name():
    ...
    ref1 = np.array(bateman_full(
        [y0[n] for n in newode.names],
        [p_vals['s'] - 1, p_vals['s'] + 1, p_vals['r']],
        res1.xout - res1.xout[0], exp=np.exp
    )).T
    assert np.allclose(res1.yout, ref1)
